...
const handleSignup = async(req, res, next)=>{
    try{
        ...
        res.cookie("access_token", jwtToken,{httpOnly: true, maxAge: 1000*60*60*24*30, secure: true, sameSite: 'None', domain: 'makeurart.vercel.app'}).status(201).json(rest);
    }
    catch(err){
        if(err.code === 11000){
...
const handleLogin = async(req, res, next)=>{
    try{
        ...
        const {password: pass, ...rest} = userData._doc;
        const jwtToken = jwt.sign({ id: userData._id}, process.env.JWT_SECRET);
        res.cookie("access_token", jwtToken,{httpOnly: true, maxAge: 1000*60*60*24*30, secure: true, sameSite: 'None', domain: 'makeurart.vercel.app'}).status(200).json(rest);
    }
    catch(err){
        next(err);
    }
